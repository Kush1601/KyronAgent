...
export function VogenApp() {
  ...
  const setup = async (audioBlob: Blob) => {
    ...
    try {
      ...
      setStep("ready");
      notify("Your vogen agent is ready! 🎉", "success");
    } catch (e) {
  ...
  const onRecord = (audioBlob: Blob) => {
    setBlob(audioBlob);
    lock.current = false;
    notify("Recording saved — building your vogen agent…", "info");
  };
      ...
      <div className="relative z-10 max-w-[900px] mx-auto px-6 pb-20">
        <header className="pt-12 pb-8 mb-10 border-b border-white/10">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-5xl font-bold text-green-700 mb-2">vogen</h1>
              <p className="text-gray-600 text-sm">
                Your Personalized AI Voice Agent
              </p>
            </div>
